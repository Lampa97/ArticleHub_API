...
def test_register_user_duplicate(client):
    ...
    """
    user_data = {"email": "dupeuser@example.com", "name": "Dupe User", "password": "dupepassword"}
...
def test_login_for_access_token(client):
    ...
    """
    user_data = {"email": "loginuser@example.com", "name": "Login User", "password": "loginpassword"}
    with patch("services.tasks.send_welcome_email.delay"):
        client.post("/api/v1/auth/register/", json=user_data)
...
def test_login_invalid_password(client):
    ...
    """
    user_data = {"email": "badpass@example.com", "name": "Bad Pass", "password": "goodpassword"}
    with patch("services.tasks.send_welcome_email.delay"):
        client.post("/api/v1/auth/register/", json=user_data)
...
def test_get_profile(client):
    ...
    """
    user_data = {"email": "profileuser@example.com", "name": "Profile User", "password": "profilepassword"}
    with patch("services.tasks.send_welcome_email.delay"):
        client.post("/api/v1/auth/register/", json=user_data)
    login_data = {"username": user_data["email"], "password": user_data["password"]}
    response = client.post("/api/v1/auth/login/", data=login_data)
